...
namespace CustomerPortal.FileLoader.TimerJob.FileLoader
{
    ...
    public class FileLoader : SPJobDefinition
    {
        ...
        static string renameFile(string fileName)
        {
            ...
            try
            {
                ...
                if (positionOfPeriod != -1)
                    ext = fileName.Substring(positionOfPeriod);

                string[] array = fileName.Split(';');
                if (array[4] != null)
                    newFileName = array[4] + ext;
            }
            catch
            {
                    ...
                    if (System.IO.File.Exists(@filePath))
                    {
                        try
                        {
                            ...
                            if (list != null)
                            {
                                ...
                                try
                                {
                                    string dateString = getReportDate(fileName);
                                    int positionOfPeriod = dateString.LastIndexOf('.');
                                    if (positionOfPeriod != -1)
                                        dateString = dateString.Substring(0, positionOfPeriod);
                                    DateTime date = DateTime.ParseExact(dateString, "MMddyyyy", System.Globalization.CultureInfo.InvariantCulture);
                                    item["ReportDate"] = date.ToShortDateString();
                                }
                                catch { }
                                ...
                                else
                                {
                                    ...
                                    Log.CsvWriter.CreateCsvFile(fileInfo, path);
        ...
        private static void fileNotUploaded(string filePath)
        {
            ...
            Log.CsvWriter.CreateCsvFile(unloadedFiles, path);
